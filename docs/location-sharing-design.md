...
A working scaffold of the relay server (Go, stdlib only) and the crypto core (Rust, `identity`/`pairing`/`envelope` modules implementing sections 3-5 of this doc) has been built against this design: server in `server/`, crypto core in `crypto-core/`. The server compiles and was exercised end to end (register, challenge/verify, PUT/GET-inbox/DELETE on the relay, auth rejection). The Rust crypto core could not be compiled where it was written: `ml-kem`/`ml-dsa` require Rust 1.85+/edition2024, and that environment only had 1.75 available. Run `cargo build && cargo test` locally first, and grep for `TODO(verify)` in `crypto-core/src/` for the handful of method names that came from documentation text rather than a real compile.
